...
- Frontend sends data to the backend through `POST /submit`
...
Input → API → Multi Judge Evaluation → Critique Round → Final Decision
...
The interface acts as the entry point to a multi agent AI safety evaluation pipeline. The system provides structured and explainable safety assessments before deployment.
